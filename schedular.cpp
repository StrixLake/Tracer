...
cl_event schedule_work(cl_resource* resource, cl_kernel* kernel, cl_event event){
    ...
    for(size_t i = 0; i < groups; i += wgp){
        global_work_offset[0] = i;
        int_ret = clEnqueueNDRangeKernel(resource->queue, *kernel, 3, (const size_t*)global_work_offset, global_work_size, local_work_size, 1, &event, &ev);
        event = ev;
        clSetEventCallback(ev, CL_COMPLETE, work_complete, (void*)(i/groups));
    }
